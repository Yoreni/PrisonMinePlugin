...
public class PriceInfo {
	...
	public PriceInfo(Shop shop) {
		...
		Map<String, String> section = QuickSell.cfg.getMapParameterized("shops." + shop.getID() + ".price");
		...
		for (String item: prices.keySet()) {
			...
			{
				info.put(item, new ItemBuilder(Material.getMaterial(item))
						.setName("&r" + StringUtils.formatItemName(new ItemStack(Material.getMaterial(item)), false))
						.setLore("", "&7Worth (1): &6" + DoubleHandler.getFancyDouble(getPrices().get(item)), "&7Worth (64): &6" + DoubleHandler.getFancyDouble(getPrices().get(item) * 64)).toItemStack());
				order.add(item);
				return;
			}
