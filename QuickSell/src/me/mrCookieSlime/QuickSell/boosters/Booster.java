...
public class Booster {
	...
	public Booster(int id) throws ParseException {
		active.add(this);
		this.id = id;
		this.cfg = new Yaml(String.valueOf(id), QuickSell.getInstance().PLUGIN_FOLDER + "data-storage/boosters");
		if (cfg.contains("type")) this.type = BoosterType.valueOf(cfg.getString("type"));
		else {
			cfg.set("type", BoosterType.MONETARY.toString());
			//cfg.save();
		...
		{
			for (Object key: cfg.getMapParameterized("contributors").keySet())
			{
				if(key instanceof String)
				{
					String k = (String) key;
					contributors.put(k, cfg.getInt("contributors." + k));
				}
			}
		}
		else
	...
	private void writeContributors() {
		for (Map.Entry<String, Integer> entry: contributors.entrySet()) {
			cfg.set("contributors." + entry.getKey(), entry.getValue());
		}
	...
	public void activate() {
		...
		if (!infinite)
		{
			...
			cfg.set("owner", getOwner());
			cfg.set("multiplier", multiplier);
			cfg.set("minutes", minutes);
			cfg.set("timeout", new SimpleDateFormat("yyyy-MM-dd-HH-mm").format(timeout));
			cfg.set("private", this instanceof PrivateBooster ? true: false);
	...
	{
		List<Booster> expiredBoosters = new ArrayList<Booster>();

		for(int i = 0; i < active.size() ; i++)
		{
			Booster booster = active.get(i);
		...
		active.removeAll(expiredBoosters);
	}
